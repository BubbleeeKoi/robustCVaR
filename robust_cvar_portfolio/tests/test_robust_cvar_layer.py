...
def test_kappa_one_equals_fractional_cvar():
    losses = np.array([0.01, 0.02, 0.03, 0.04, 0.20, 0.25, 0.30, 0.40, 0.50, 0.60])
    from robust_cvar_portfolio.src.risk_metrics import cvar_alpha_fractional

    frac = cvar_alpha_fractional(losses, alpha=0.1)
    rcvar = robust_cvar(losses, 1.0, alpha=0.1)
    assert abs(frac - rcvar) < 1e-10
